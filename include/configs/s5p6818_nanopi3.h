...
#define CONFIG_SYS_MEM_SIZE		0x80000000  //0x40000000 0x80000000
...
#define CONFIG_SYS_MALLOC_LEN		(32*1024*1024)  //32M堆大小
...
/* dram 2 bank num */
#define CONFIG_NR_DRAM_BANKS		2  //2 ? maybe BANKS=2**2

/* kernel load address */
#define CONFIG_SYS_LOAD_ADDR		0x40080000 //0x48000000 0x40080000
#if 0
#define CONFIG_KERNEL_DTB_ADDR		0x49000000  //0x49000000 0x4A000000
#endif
/* fastboot buffer start, size */
#define CONFIG_FASTBOOT_BUF_ADDR	0x4A000000 //0x4A000000  0x4B000000
...
#define CONFIG_SYS_MMC_DEV		1 // 0=板载MMC/c0069000 1=SD/c0062000 arch/arm/dts/s5p6818-nanopi3.dts
#define CONFIG_2NDBOOT_OFFSET		512  // 2ndboot 起始地址
#define CONFIG_2NDBOOT_SIZE		(64*1024)  // 2ndboot size 128个扇区
#define CONFIG_FIP_OFFSET		(CONFIG_2NDBOOT_OFFSET + CONFIG_2NDBOOT_SIZE) //129*512 129扇区UBOOT含nsih
#define CONFIG_FIP_SIZE			(3*1024*1024) //(2880*1024) //uboot size 实际大小约500K
#define CONFIG_ENV_IS_IN_MMC            // 环境变量永久保存于MMC设备中
#define CONFIG_SYS_MMC_ENV_DEV		CONFIG_SYS_MMC_DEV  // 0=保存于板载MMC设备 1=SD卡
#define CONFIG_ENV_OFFSET		(CONFIG_FIP_OFFSET + CONFIG_FIP_SIZE) //环境变量设置永久存储扇区 uboot之后
...
#define CONFIG_EXTRA_ENV_DTB_RESERVE				\
	...
	"if test -n \"$fb_addr\"; then "			\
	  ...
	  "fdt set /reserved-memory/display_reserved reg <$fb_addr 0xc00000>;"	\
	"fi;\0"
#endif
...
#define CONFIG_EXTRA_ENV_SETTINGS				\
	...
	"fdtfile=s5p6818-nanopim3.dtb\0"			\
	"rootdev=" __stringify(CONFIG_ROOT_DEV) "\0"		\
	"rootpart=" __stringify(CONFIG_ROOT_PART) "\0"		\
	"bootpart=" __stringify(CONFIG_BOOT_PART) "\0"		\
	"load_fdt=ext4load mmc $rootdev:$bootpart $fdtaddr $fdtfile\0"	\
	"load_kernel=ext4load mmc $rootdev:$bootpart $kerneladdr $kernel\0"	\
	"boot_cmd_mmcboot=\n"						\
		"run load_kernel; run load_fdt; "	        \
		"booti $kerneladdr - $fdtaddr\0"	        \
	"bootcmd=run boot_cmd_mmcboot\0"			\
	CONFIG_EXTRA_ENV_BOOT_LOGO
#if 0	
#define CONFIG_EXTRA_ENV_SETTINGS	\
...
#define CONFIG_BOOTCOMMAND	\
	"ext4load mmc 1:1 0x40080000 Image; " \
	"ext4load mmc 1:1 0x4A000000 s5p6818-nanopim3.dtb; " \
	"booti 0x40080000 - 0x4A000000"
#endif
#endif /* __CONFIG_H__ */
